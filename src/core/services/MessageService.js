...
export default class MessageService {
  ...
  async createIncomingMessage(input = {}, companySettings = {}, instance = {}, company = {}) {
    ...
    if (input.data.messageType === "documentMessage") {
      const brokerUrl = input.data.message.documentMessage.url;
      ...
      const localPath = await this.downloaderFile.downloadFile(brokerUrl, filename);
      ...
      const brokerUrl = input.data.message.documentWithCaptionMessage.message.documentMessage.url;
      ...
      const brokerUrl = input.data.message.audioMessage.url;
      ...
      const brokerUrl = input.data.message.videoMessage.url;
    ...
    const message = Message.newIncoming(companySettings.id, instance.id, protocol.id, input.data.key.id, content, messageType, contentType);
